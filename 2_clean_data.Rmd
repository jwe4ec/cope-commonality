...
cope_full_data <- readRDS(file.path("data", "source", "cleaned_cope_data_randomized_rev1.rds"))
```
...
#   Given that a demographics column is different in the "cope_ca_ssi_completers" object based on the current
#   (vs. the prior) source data, the imputation code, data, and results below may not be fully reproducible.
...
write.csv(cope_ca_data_nonimputed, file.path(data_processed_path, "cope_ca_data_nonimputed.csv"))
```
...
sink(file.path(session_info_path, "2026.06.26_sessionInfo_for_cleaning_script.txt"))
sessionInfo()
sink()
```
